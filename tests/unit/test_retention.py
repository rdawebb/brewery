...
class TestCleanupCandidates:
    ...
    def test_recent_sidecar_excluded(self, tmp_path, make_keg) -> None:
        """Test that a recent sidecar is excluded."""
        cellar = tmp_path / "Cellar"
        mark_replaced(make_keg(cellar, "wget", "1.0"), by="2.0", at=1000)
        assert cleanup_candidates(cellar, active=set(), now=1000 + 10 * DAY) == []

    def test_sidecarless_excluded(self, tmp_path, make_keg) -> None:
        """Test that a sidecarless keg is excluded."""
        cellar = tmp_path / "Cellar"
        make_keg(cellar, "wget", "1.0")  # No sidecar
        assert cleanup_candidates(cellar, active=set(), now=1000 + 99 * DAY) == []

    def test_active_excluded(self, tmp_path, make_keg) -> None:
        """Test that an active keg is excluded."""
        cellar = tmp_path / "Cellar"
        k = make_keg(cellar, "wget", "1.0")
