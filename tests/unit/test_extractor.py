"""Tests for the bottle extractor.

Tars are built in memory and compressed as gzip and zstd. Happy-path, mode, and
symlink tests run against both formats; safety and layout edge cases use gzip
for brevity. The key fidelity checks are that read-only bottle modes survive
extraction (the stock data filter would clamp them) and that placeholder
symlink targets are not mistaken for unsafe absolute links.
"""
...
from brewery.providers.extractor import ExtractionError, detect_format, extract_bottle
...
def test_detect_format(tmp_path, fmt, compress) -> None:
    """Test format detection."""
    arc = _archive(tmp_path, compress, make_tar(standard_keg()))
    assert detect_format(arc) == fmt
