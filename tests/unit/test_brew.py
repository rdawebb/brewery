...
def _mock_run(monkeypatch, result: BrewResult) -> list[dict]:
    ...
    async def Mock(args, *, output, check) -> BrewResult:
...
async def test_formula_install_passes_formula_flag(monkeypatch) -> None:
...
async def test_cask_install_passes_cask_flag(monkeypatch) -> None:
...
async def test_upgrade_passes_no_kind_flag(monkeypatch) -> None:
...
async def test_already_installed_maps_to_warning(monkeypatch) -> None:
...
async def test_pinned_upgrade_maps_to_warning(monkeypatch) -> None:
...
async def test_generic_nonzero_raises_command_error(monkeypatch) -> None:
...
async def test_already_installed_message_on_success_is_not_warning(monkeypatch) -> None:
